...
| `0002_delete_legacy_supports.sql` | Data migration: deletes every legacy `claim_relation` SUPPORTS row — their only writer was the pre-restructure cross-ref promotion branch (wrong direction claim→neighbor, weight=verdict-certainty, similarity provenance). Regenerable graph context; `migrate.test.ts` seeds and asserts the wipe (contradicts rows retained).                                 |
