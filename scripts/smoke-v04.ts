...
async function main(): Promise<void> {
  ...
  if (colNames.length !== 7) throw new Error("missing verifiability columns");
  ...
  const edgesWritten = await writeClaimEdges(claimAId, [claimBId], "contradicts", {
    weight: 0.9,
    createdBy: "auto",
    metadata: { source: "smoke" },
  });
  console.log(`[3] writeClaimEdges(contradicts): edges_inserted=${edgesWritten}`);
  if (edgesWritten !== 1) throw new Error("edge not inserted");

  // Idempotency — re-write should hit ON CONFLICT DO NOTHING
  const edgesAgain = await writeClaimEdges(claimAId, [claimBId], "contradicts");
  console.log(`[3a] re-write idempotency: edges_inserted=${edgesAgain} (expect 0)`);
  if (edgesAgain !== 0) throw new Error("duplicate edge inserted");
  ...
  const bundles = await fetchFactBundlesForEntries(
    [{ id: entryId, createdAt: entryCreatedAt.toISOString() }],
    { maxPerEntry: 5 },
  );
  ...
  if ((bundleA?.contradicts.length ?? 0) !== 1) throw new Error("A should surface outgoing CONTRADICTS to B");
  if ((bundleB?.contradicts.length ?? 0) !== 1) throw new Error("B should surface incoming CONTRADICTS from A");

  // 5. claim_feedback insert → claim.authority moves
  const beforeAuth = (await db.select({ a: claim.authority }).from(claim).where(eq(claim.id, claimAId)).limit(1))[0]!.a;
  ...
  if (!("ok" in fbResult) || !fbResult.ok) throw new Error("feedback failed");
  const afterAuth = (await db.select({ a: claim.authority }).from(claim).where(eq(claim.id, claimAId)).limit(1))[0]!.a;
  console.log(`    claim.authority: ${beforeAuth.toFixed(4)} → ${afterAuth.toFixed(4)}  (Δ=${(afterAuth - beforeAuth).toFixed(4)})`);
  if (afterAuth >= beforeAuth) throw new Error("authority should drop on failed feedback");
  ...
  if (!("ok" in fbUpdate) || !fbUpdate.ok || !fbUpdate.updated) throw new Error("update mode failed");

  // 7. authority-learn EMA on a verdict transition
  const learnResult = await recordVerdictTransition(claimAId, "verified", "disputed");
  console.log("[7] authority-learn:", JSON.stringify(learnResult));
  const repRow = await db
    .select({ fa: agentFeedbackAuthority.feedbackAuthority, total: agentFeedbackAuthority.totalFeedbacks, correct: agentFeedbackAuthority.correctFeedbacks })
  ...
  if (!repRow[0] || repRow[0].correct !== 1) throw new Error("verdict transition should have credited reporter");
  ...
  if ("ok" in mismatchResult && mismatchResult.ok) throw new Error("mismatch should have been rejected");

  // Cleanup
  await db.delete(claimRelation).where(eq(claimRelation.sourceClaimId, claimAId));
  await db.delete(claimFeedback).where(eq(claimFeedback.claimId, claimAId));
  await db.delete(entry).where(eq(entry.id, entryId));
  await db.delete(agentFeedbackAuthority).where(eq(agentFeedbackAuthority.agentId, "smoke-agent"));
