...
export const claim = pgTable(
  ...
  (t) => [
    ...
    check(
      "claim_polarity_values",
      sql`${t.polarity} IS NULL OR ${t.polarity} IN (0, 1)`,
    ),
    check(
...
export const claimRelation = pgTable(
  ...
  (t) => [
    ...
    check(
      "claim_relation_no_self_loop",
      sql`${t.sourceClaimId} <> ${t.targetClaimId}`,
    ),
    uniqueIndex("uniq_claim_relation_edge").on(
      t.sourceClaimId,
      t.targetClaimId,
      t.relationType,
    ),
...
export const goldenSetClaim = pgTable(
  ...
  (t) => [
    ...
    check(
      "golden_set_claim_type_values",
      sql`${t.claimType} IN ('factual', 'subjective', 'predictive', 'normative')`,
    ),
    check(
...
export const goldenSetRun = pgTable(
  ...
  (t) => [
    ...
    check(
      "golden_set_run_precision_range",
      sql`${t.precisionOverall} >= 0 AND ${t.precisionOverall} <= 1`,
    ),
    check(
      "golden_set_run_recall_range",
      sql`${t.recallOverall} >= 0 AND ${t.recallOverall} <= 1`,
    ),
    check(
      "golden_set_run_f1_range",
      sql`${t.f1Overall} >= 0 AND ${t.f1Overall} <= 1`,
    ),
    check(
      "golden_set_run_regressed_values",
      sql`${t.regressed} IS NULL OR ${t.regressed} IN (0, 1)`,
    ),
...
export const claimFeedback = pgTable(
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.outcome} IN ('held','failed','partial')`,
    ),
    check(
    ...
    check(
      "claim_feedback_evidence_strength_range",
      sql`${t.evidenceStrength} >= 0 AND ${t.evidenceStrength} <= 1`,
    ),
    check(
    ...
    check(
      ...
      sql`${t.reporterResponded} IS NULL OR ${t.reporterResponded} IN (0, 1)`,
    ),
    check(
      "claim_feedback_audit_note_len",
      sql`${t.auditNote} IS NULL OR length(${t.auditNote}) <= 4000`,
    ),
    check(
      "claim_feedback_counter_source_url_len",
      sql`${t.counterSourceUrl} IS NULL OR length(${t.counterSourceUrl}) <= 2000`,
    ),
    check(
...
export const agentFeedbackAuthority = pgTable(
  ...
  (t) => [
    check(
      "agent_feedback_authority_range",
      sql`${t.feedbackAuthority} >= 0 AND ${t.feedbackAuthority} <= 1`,
    ),
    ...
    check(
      "agent_feedback_consistency",
      sql`${t.correctFeedbacks} + ${t.incorrectFeedbacks} <= ${t.totalFeedbacks}`,
    ),
