...
import type { FeedbackReason } from "../a2a/handlers/feedback";
...
// already committed.
export async function routeFeedbackAction(input: RouteInput): Promise<void> {
  ...
  switch (input.reason) {
    ...
    case "missing":
      ...
      logger.info(
        { entryId: input.entryId, agentId: input.agentId, note: input.note },
        "feedback: missing knowledge gap noted",
      );
      return;
    case "used":
    case "helpful":
...
async function reverifyEntryClaims(
  entryId: string,
  entryCreatedAt: Date,
): Promise<void> {
  ...
  logger.info(
    { entryId, claimsRequeued: claims.length },
    "feedback wrong: claims requeued for re-verification",
  );
}
