...
import {
  normalizeTokenizerSpacing,
  isSemanticGarbage,
  hostPasses,
  estimateSourceType,
} from "../../src/collect/research";
...
describe("isSemanticGarbage", () => {
  ...
  test("Chinese prose passes", () => {
    const prose =
      "深度 学习 是 机器 学习 的 一个 分支 。 它 使用 多 层 神经 网络 来 处理 复杂 的 模式 识别 任务 。 ".repeat(6);
    expect(isSemanticGarbage(prose)).toBe(false);
  });
...
describe("estimateSourceType", () => {
  test("arxiv → research_paper", () => {
    expect(estimateSourceType("https://arxiv.org/abs/2404.10774")).toBe("research_paper");
  });

  test("openreview → research_paper", () => {
    expect(estimateSourceType("https://openreview.net/forum?id=abc")).toBe("research_paper");
  });

  test("learn.microsoft → official_docs", () => {
    expect(estimateSourceType("https://learn.microsoft.com/en-us/azure")).toBe("official_docs");
  });

  test("docs.python → official_docs", () => {
    expect(estimateSourceType("https://docs.python.org/3/library/asyncio.html")).toBe("official_docs");
  });

  test("wikipedia → reference_wiki", () => {
    expect(estimateSourceType("https://en.wikipedia.org/wiki/Transformer")).toBe("reference_wiki");
  });

  test("github README → official_docs (path-aware)", () => {
    expect(estimateSourceType("https://github.com/owner/repo/blob/main/README.md")).toBe("official_docs");
  });

  test("github issues → community_forum (path-aware)", () => {
    expect(estimateSourceType("https://github.com/owner/repo/issues/42")).toBe("community_forum");
  });

  test("github releases → github_release", () => {
    expect(estimateSourceType("https://github.com/owner/repo/releases/tag/v1.0")).toBe("github_release");
  });

  test("github root → github_release (default)", () => {
    expect(estimateSourceType("https://github.com/owner/repo")).toBe("github_release");
  });

  test(".gov → official_docs", () => {
    expect(estimateSourceType("https://nist.gov/x")).toBe("official_docs");
  });

  test("medium → established_blog", () => {
    expect(estimateSourceType("https://medium.com/@author/post")).toBe("established_blog");
  });

  test("stackoverflow → community_forum", () => {
    expect(estimateSourceType("https://stackoverflow.com/q/123")).toBe("community_forum");
  });

  test("unknown host → unknown", () => {
    expect(estimateSourceType("https://random-blog.example.io/post")).toBe("unknown");
  });

  test("malformed url → unknown", () => {
    expect(estimateSourceType("not a url")).toBe("unknown");
  });
});
